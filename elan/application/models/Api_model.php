 ...
 */
  ...
  public function getActiveJobsList(){
     ...
     if(!empty($active)){
       ...
       // $name = (int) $active['id'] + 1;
       // return 'A'.$name;
          $getGata = $this->db->get('admin_master')->row_array();
          $count = $getGata['job_count_id'] + 1;
          $this->db->where('id',1);
          $this->db->update('admin_master',array('job_count_id'=>$count));
         //$name = (int) $active['id'] + 1;
        return 'A'.$count;
     }else{
       return 'A1';
  ...
  public function getJobsList($request){
       ...
       if($request['type'] == 1){
        ...
        isBlank($request['user_id'],NOT_EXISTS,125);
            ...
            if(empty($getJobName)):
               $getJobName  = $this->getActiveJobsList();
          endif;
         // $addJob['job_name']   = '';
          if(!empty($getJobName)):
 ...
 */
  public function jobUserList($request){
     ...
     $offset = (empty($request['offset']) ? 0 : $request['offset']);
      $limit = 10;
       ...
       jobs_master.pdf_url,jobs_master.delivery_date,jobs_master.add_date,jobs_master.status_seen,jobs_master.job_status FROM `project_master` INNER JOIN jobs_master ON jobs_master.projetc_id = project_master.id 
       WHERE project_master.status = 1 AND jobs_master.status = 1 
       AND jobs_master.projetc_id = '.$request['project_id'].'  AND jobs_master.delivery_date IS NOT NULL ORDER BY jobs_master.id DESC LIMIT '.$offset.','.$limit;
       $getData = $this->db->query($sql)->result_array();
       $result = array();
       foreach($getData as $list):
              $list['factory'] = self::getFactoryDetails($list['job_id']);
              $result[] = $list;
       endforeach;
       //$getData = $this->db->get_where('jobs_master',array('status'=>1,'user_id'=>$request['user_id']))->result_array();
      // echo $this->db->last_query();exit;
       $response['data']   = $result;
       $response['offset'] = count($result) + $request['offset'];
  ...
  public function getFactoryDetails($getJobId){
     $this->db->select('tbl_assign_jobs.factory_id,tbl_assign_jobs.job_status,factory_master.name,factory_master.email');
     $this->db->from('tbl_assign_jobs');
     $this->db->join('factory_master','factory_master.id=tbl_assign_jobs.factory_id','inner');
     $this->db->where('tbl_assign_jobs.job_id',$getJobId);
 ...
 */
    ...
    <table style="margin-top:18px;">
     ...
     <td>
      <div style="width:350px;">
        ...
        <h6 style="margin: 0px;padding-bottom: 10px;font-size: 13px;padding-left: 15px;">7051CM Varsseveld &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;7050AA Varsseveld</h6>
  ...
  // New tab Add
    ...
    if(!empty($checkData)){
        ...
        foreach($checkData as $list){
         $html .= '<div style="width: 50%;float: left;">
               <div style="width:350px;float: left; border: 1px solid; height:120px;">
                <div style="border:1px solid black; height:26px; width:28px;position: absolute; text-align:center;">'.$count.'</div>
               <center>';
              $html .= '<div style="margin:0 auto;padding-top:10px;width:230px;height:215px;"><img src="'.base_url().'/uploads/pdf/'.$list['images'].'" width="230px" height="215px"></div></center>';
               ...
               <div style="width:32.5%;border: 1px solid;float: left;">&nbsp;&nbsp;L: '.$list['length'].(!empty($list['length']) ? 'mm':'').'</div>
        ...
        $this->db->where('id',$request['job_id']);
        $this->db->update('jobs_master',array('pdf_url'=>$pdfName,'delivery_date'=>$request['delivery_date'],'modify_date'=>time()));
        ...
        foreach($checkData as $list){ 
            ...
            if(!empty($jobDetails['companyEmail'])):
                 ...
                 $subject = $jobDetails['projetc_number'].', '.$jobDetails['project_name'].'-'.$jobDetails['job_name'];
  ...
  public function getProjectDetails($request){
     $this->db->select('project_master.*,project_master.company_id,project_master.projectStatus,(select count(id) from jobs_master where projetc_id = project_master.id) as totalJobs,(select count(user_assign_project.id) from user_assign_project INNER JOIN user_master on user_master.id = user_assign_project.user_id  where user_assign_project.project_id = project_master.id AND user_master.status=1) as totalUsers');
     $result = $this->db->get_where('project_master',array('id'=>$request['projectid']))->row_array();
     //print_r($result);exit;
       $getResult = array();
       //foreach($result as $list){
              $company = $this->db->get_where('manage_company',array('id'=>$result['company_id']))->row_array();
              $data['id'] = $result['id'];
              $data['company']     = $company['name'];
              $data['projectName'] = $result['project_name'];
              $data['projectNumber'] = $result['projetc_number'];
              $data['address']       = $result['address'];
              $data['projectImage']  = (!empty($result['projectImage']) ? base_url('uploads/project/'.$result['projectImage']) : '');
              $data['addDate']       = $result['add_date'];
              $data['totalJob']      = $result['totalJobs'];
              $data['totalUser']     = $result['totalUsers'];
              $data['projectStatus'] = $result['projectStatus'];
              $data['files']         = self::getAllFiles($result['id']);
             // $getResult[] = $data;
       //}
       $response['fileUrl'] =  base_url('uploads/document/');
  ...
  public function getAllFiles($projectId){
       return $this->db->get_where('project_document',array('project_id'=>$projectId))->result_array();
  }
    public function updateProjectImages($request){
       if(!empty($request['projectImage'])){
           $getImage = saveProjectProfilesImage($request['projectImage']);
           if(!empty($getImage)):
             $this->db->where('id',$request['projectId']);
             $this->db->update('project_master',array('projectImage'=>$getImage));
           endif;
           generateServerResponse(S,SUCCESS);
       }else{
         generateServerResponse(S,SUCCESS);
    ...
    public function projectGallery($request){
    //gallery
       if(!empty($request['projectImage'])){
           $getImage = saveProjectGalleryImage($request['projectImage']);
           if(!empty($getImage)):
                  $insert = array();
                  $insert['user_id']     = $request['user_id'];
                  $insert['project_id']  = $request['project_id'];
                  $insert['user_type']   = 2;
                  $insert['imageUrl']    = $getImage;
                  $insert['add_date']    = time();
                  $this->db->insert('project_gallery',$insert);
           endif;
           generateServerResponse(S,SUCCESS);
       }else{
         generateServerResponse(S,SUCCESS);
  ...
  public function DeleteProjectGallery($request){
    //gallery
        if($request['type'] == 1):
         $this->db->where('id',$request['gallery_id']);
         $this->db->delete('project_gallery');
         generateServerResponse(S,SUCCESS);
       else:
         $result = $this->db->get_where('project_gallery',array('project_id'=>$request['project_id']))->result_array();
          if(!empty($result)):
               $getResult = array();
               foreach($result as $list):
                      $get['id']         = $list['id'];
                      $get['project_id'] = $list['project_id'];
                      $get['imageUrl']   = (!empty($list['imageUrl']) ? base_url('uploads/gallery/'.$list['imageUrl']) :'');
                      $getResult[] = $get;
               endforeach;
               /* Response*/
                if(!empty($getResult)):
